...
describe('Xlsx Wallet Spreadsheet Extractor Adapter', () => {
  ...
  it('should throw InvalidWalletSpreadsheetException when spreadsheet is invalid', () => {
    const sut = new XlsxWalletSpreadsheetExtractorAdapter();

    expect(() => sut.extract(Buffer.alloc(0))).toThrow(
      InvalidWalletSpreadsheetException,
    );
  });
});
