...
/**
 * Creates a PrismaClient configured to connect to the given database URL
 * using the PrismaPg driver adapter.
 */
...
/**
 * Creates a NestJS TestingModule with PrismaService overridden to use
 * a PrismaClient connected to the given database URL.
 */
export async function createTestingModule(
    databaseUrl: string,
    metadata: { imports?: any[]; providers?: any[] } = {},
): Promise<{ module: TestingModule; prismaClient: PrismaClient }> {
    const prismaClient = createTestPrismaClient(databaseUrl);
